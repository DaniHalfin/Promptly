...
3. **Source panels** (one per `connected: true && error: null` source):
   ...
   - **`<CopilotPanel />`** renders (in order):
     ...
     2. **§7.15 / F7 Average tokens per session** — KPI tile (GitHub Copilot branch): renders `copilotAvgTokensPerSession` (label: "Avg Tokens / Session"). Formula: mean of `Σ(inputTokens + outputTokens)` per session across all models; Copilot total-field semantics (cache tokens are subsets, not additive). This value is measured from local `session.shutdown` event token counts and must not be labeled "estimated"; only reconstructed spend values/charts receive estimated annotations. *(F7)*
